...
def crete_contact_method(
    contact_id: int, data: ContactMethodCreate, session: Session = Depends(get_session)
):
    contact = session.get(Contact, contact_id)
    if not contact:
        raise HTTPException(status_code=404, detail="Contact not found")

    method = ContactMethod(contact_id=contact_id, type=data.type, value=data.value)
...
def update_contact_method(
    id: int, data: ContactMethodUpdate, session: Session = Depends(get_session)
):
